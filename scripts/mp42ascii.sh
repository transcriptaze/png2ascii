...
convert () {
    ...
    echo "${filename}"
    png2ascii --debug --profile "$PROFILE" --format png --out "./out/${filename}" $1
...
# Download MP4 from Google Drive
...
ffmpeg -y -i "${MP4}"  out.wav
ffmpeg -y -i "${MP4}" -f image2 -c:v png frames/frame_%05d.png
...
ffmpeg -y -r 29.92 -f image2 -i out/frame_%05d.png -pix_fmt yuv420p out.mp4
ffmpeg -y -i out.mp4 -i out.wav -c:v copy -c:a aac mp42asc.mp4
