...
func (m Png2Png) Convert(img image.Image, dest string, squoosh bool, debug bool) error {
	bounds := img.Bounds()
	w := bounds.Dx()
	h := bounds.Dy()
	...
	if squoosh {
		scaled := m.squoosh(img, m.font, debug)
...
func (m Png2Png) squoosh(img image.Image, font font.Face, debug bool) image.Image {
