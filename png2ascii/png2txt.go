...
func (m Png2Txt) Convert(img image.Image, dest string, squoosh Squoosh, debug bool) error {
	if debug {
		bounds := img.Bounds()
		w := bounds.Dx()
		h := bounds.Dy()
...
func (m Png2Txt) squoosh(img image.Image, squoosh Squoosh, debug bool) image.Image {
	bounds := img.Bounds()
	W := bounds.Dx()
	H := bounds.Dy()
